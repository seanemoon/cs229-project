...
numDisplays = min(args.numDisplays, sampleNum)

webcamList = [os.path.join(Dir, webcam) for webcam in os.listdir(Dir)]
...
while True:
  ...
  webcams.sort(key = lambda x: x.score(), reverse = True)
